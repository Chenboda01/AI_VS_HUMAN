...
class GameEngine {
    constructor() {
        ...
        this.questions = [
            ...
            { question: "What is the largest mammal in the world?", answers: ["Elephant", "Blue Whale", "Giraffe", "Polar Bear"], correct: 1 },
            { question: "Who painted the Mona Lisa?", answers: ["Van Gogh", "Picasso", "Da Vinci", "Rembrandt"], correct: 2 },
            ...
            { question: "Which element is the most abundant in Earth's atmosphere?", answers: ["Oxygen", "Carbon", "Nitrogen", "Hydrogen"], correct: 2 },
            { question: "Who wrote 'Romeo and Juliet'?", answers: ["Charles Dickens", "William Shakespeare", "Mark Twain", "Jane Austen"], correct: 1 },
            { question: "What is the speed of light in vacuum (m/s)?", answers: ["299,792,458", "300,000,000", "299,792", "299,792,459"], correct: 0 }
        ];
    ...
    nextTurn() {
        ...
        this.logEvent(`Turn ${this.state.currentTurn}: ${this.state.currentPlayer.toUpperCase()}'s turn`);
        return true;
    }
    ...
    answerQuestion(answerIndex) {
        ...
        if (isCorrect) {
            ...
            this.logEvent(`Human answered correctly! +${points} points. Knowledge increased.`);
        } else {
    ...
    sendTroops(humanTroopsCount) {
        ...
        if (this.players.ai.houseDefended) {
            ...
            this.logEvent(`Human sent ${troops} troops! AI's defense reduced damage by 50%.`);
        } else {
    ...
    aiAnswerQuestion() {
        ...
        if (isCorrect) {
            ...
            this.logEvent(`AI answered correctly! +${points} points. Knowledge increased.`);
        } else {
    ...
    aiSendTroops() {
        ...
        if (this.players.human.houseDefended) {
            damage = Math.floor(damage * 0.5);
            this.logEvent(`AI sent ${troops} troops! Human's defense reduced damage by 50%.`);
        } else {
    ...
    updateAIStrategyWeights(strategy, result) {
        ...
        const total = this.aiStrategyWeights.answer + this.aiStrategyWeights.attack + this.aiStrategyWeights.defend;
    ...
    endGame() {
        ...
        if (this.players.human.score > this.players.ai.score) {
            ...
            this.logEvent(`Game over! Human wins with ${this.players.human.score} points!`);
            ...
            this.logEvent(`Game over! It's a draw with ${this.players.human.score} points each!`);
        }
    }
